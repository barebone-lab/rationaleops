...
const ScreenshotScene = ({ duration, file, title }: { duration: number; file: string; title: string }) => {
  const frame = useCurrentFrame();
  return (
    <Scene duration={duration}>
      <h2 style={{ ...headlineStyle, fontSize: 72, marginTop: 15, ...enter(frame, 16) }}>{title}</h2>
      <div style={{ flex: 1, overflow: "hidden", border: `2px solid ${COLORS.line}`, backgroundColor: "#0b100c", padding: 16, ...enter(frame, 30) }}>
        <Img
          src={staticFile(file)}
          style={{
            width: "100%",
...
export const RationaleOpsVideo = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.ink }}>
      ...
      {place(scenes.artifactScreenshot, <ScreenshotScene duration={scenes.artifactScreenshot} file="dashboard-action.png" title="Owner-confirmed intent becomes a passing test." />)}
